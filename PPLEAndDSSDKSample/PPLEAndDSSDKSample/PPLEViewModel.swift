...
@MainActor
final class PPLEViewModel: ObservableObject {
    ...
    private func authenticateUser() async {
        ...
        do {
            try await Authentication.shared.login(
                username: PPLECredentials.username, password: PPLECredentials.password
            )
            await getStores()
        } catch {
    ...
    private func initializeCaptureViewAndPriceCheck() {
        ...
        let priceCheck = PriceCheck(
            productCatalog: productCatalog, captureView: captureView
        )
...
extension PPLEViewModel: PriceCheckListener {
